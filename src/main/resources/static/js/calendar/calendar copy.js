...
const createCalendar = () => {
    ...
    const dates = [];
    for(let i=1; i<=thisMonthLast.getDate()-1; i++) {
        dates.push(i);
        dates[0] = `${selectedYear}/${selectedMonth+1}/1`;
        dates[i] = `${selectedYear}/${selectedMonth+1}/${i+1}`;
    }
    ...
    dates.forEach((ele, idx, arr)=> {
        dates[idx] = `<div class="calendar__date">${arr.indexOf(ele)+1}</div>`;
